...
### Fixed
- **`normalizedHotkey` stale on proc replacing position 1 (regression since 4.4.0):** The 4.4.0 three-tier queue sort (`procced → normal → on-cooldown`) causes spell positions to shift more aggressively, briefly leaving slots empty before a proc fills them. In the empty-slot clear path, `cachedNormalizedHotkey` was correctly set to `nil` but `normalizedHotkey` was not cleared. When the slot refilled, `hotkeyChanged` would see the stale `normalizedHotkey` and incorrectly write it to `previousNormalizedHotkey`, corrupting the key-press flash grace period. Fixed by also setting `icon.normalizedHotkey = nil` in the empty-slot clear path (`UIRenderer.lua`).
- **Controller button glyphs reverting to keyboard text after a few minutes:** `OnKeybindsChanged` (fired by `UPDATE_BINDINGS`) was immediately wiping `spellHotkeyCache` via the cascading `InvalidateBindingCache → InvalidateKeybindCache` call. The very next render frame called `GetBindingKey` before WoW's gamepad bindings had finished committing, caching keyboard text (or an empty string) into `spellHotkeyCache`. With `spellHotkeyCacheValid = true` restored after that lookup, the fast path served the bad result until the next hard invalidation. The 0.3s delayed refresh was designed to fix this but ran too late. Fixed by deferring `spellHotkeyCache` invalidation entirely to the 0.3s timer: the immediate path now only soft-invalidates the raw `bindingKeyCache`, so callers continue seeing cached glyphs during the gamepad-binding settling window (`ActionBarScanner.lua`).
