...
L["Situational Sets desc"] = "Three named groups of abilities, each switched in or out of the queue as one unit from a keybind. Put abilities into a set from their card on the Abilities tab. A set that is switched off hides its abilities from the queue only; the game's own live pick is never blocked, so the rotation cannot stall. Every set is switched back on when you log in or change spec."
L["Situational Set Member desc"] = "Include this ability in the set. While the set is switched off, the ability is hidden from the queue."
...
L["Set Disabled"] = "%s: |cffff6666OFF|r (hidden from the queue)"
