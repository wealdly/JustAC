...
L["Disable Blizzard Highlight desc"] = "停用此選項可提升效能。暴雪的AssistedCombatManager每幀執行一個迴圈，掃描所有動作列按鈕以顯示藍色高亮光效。JustAC已在自己的佇列中顯示相同資訊。關閉它可以消除該迴圈，並防止在插件負載較重時出現「腳本執行時間超限」錯誤。此設定跨工作階段保留。"
