...
L["Disable Blizzard Highlight desc"] = "Отключение повышает производительность. AssistedCombatManager Blizzard выполняет цикл каждый кадр, сканируя все кнопки панели действий для отображения синего эффекта. JustAC уже отображает эту информацию в собственной очереди. Отключение убирает этот цикл и может предотвратить ошибки «превышен лимит времени выполнения скрипта» при высокой нагрузке аддонов. Настройка сохраняется между сеансами."
